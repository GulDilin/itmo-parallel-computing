...
def run(
    ...
    path = f'{os.path.join(".", target.value, f"{lab_name}{suffix}")} {n_size} {k}'
...
def main(args):
    ...
    with open(config_path, 'r') as f:
        config = json.load(f)
    with open('../lab4/assets/verification.json', 'r') as f:
    ...
    try:
        for target in TargetDir:
            ...
            for n_threads in k:
                benchmarks_total = []
                for n in n_variants:
                    numbers, timing, benchmarks = run(target, n, n_threads, False)
